...
export function Brand({ size = "sm", mode = "landscape" }: BrandProps) {
  ...
  const loadAccentLogoImageSource = useCallback(async (color: AccentColor) => {
    const accentLogoSource = await swapDominantColorInRasterImage(
      logoImageSrc,
      color,
    );
  ...
  useEffect(() => {
    loadAccentLogoImageSource(accent);
  }, [accent]);
